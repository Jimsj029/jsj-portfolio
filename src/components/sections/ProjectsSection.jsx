                    ...
                    <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex items-center gap-1 z-10">
                      {getProjectImages(project).map((_, idx) => (
                        <button
                          ...
                          className={`h-2 w-2 rounded-full ${idx === (Number.isFinite(activeImageByProject[projectKey(project)]) ? activeImageByProject[projectKey(project)] : 0) ? 'bg-primary/95' : 'bg-primary/65'}`}
                          onClick={(e) => {
                            e.stopPropagation();
                            setActiveImageByProject((prev) => ({
                              ...prev,
